...
def visualize_multi_type_graph(adj_matrix, node_type_map, all_nodes, query_nodes,
    ...
    # --------------------------
    # 核心修改1：减小k值（从1.8→0.8），让节点间距缩小，连接更紧密
    # k值越小，节点分布越密集；iterations增加到300确保布局稳定
    # --------------------------
    pos = nx.spring_layout(G, seed=42, k=0.8, iterations=300)

    # 创建画布（保持18×15尺寸，足够容纳节点）
    ...
    # --------------------------
    # 核心修改2：缩小线宽（从1.2→0.4），让连接线更细
    # 同时降低alpha（从0.8→0.6），避免细线叠加后过粗
    # --------------------------
    if edge_count > 0:
        nx.draw_networkx_edges(
            G, pos,
            edge_color='darkgray',
            width=0.4,    # 线宽缩小，实现“连接线更小”的需求
            alpha=0.6     # 降低透明度，避免线条重叠显粗
        )
    ...
    plt.title("查询节点及其完整连接关系图（细线条+紧密布局）", fontsize=18, pad=25)

    # 保存图像（保持高分辨率）
    plt.savefig(save_path, dpi=200, bbox_inches='tight', facecolor='white')  # 显式设置白色背景
    plt.close()
...
def main():
    ...
    save_image_path = "thin_line_dense_layout_graph.png"  # 新文件名（区分原结果）
